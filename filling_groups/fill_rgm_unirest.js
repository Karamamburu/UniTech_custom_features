function createPositionsString(positions) {
    ...
    for (pos in positions) {
        if (positionsString == '') {
               positionsString = " AND (position_name ='" + pos +"'";
        }
        else {
             positionsString += " OR position_name='" + pos + "'"; 
        }
    }
...
try{
    ...
    INNER JOIN collaborators cl ON cl.position_parent_id = st.id AND cl.is_dismiss = 0" + sPositions;
