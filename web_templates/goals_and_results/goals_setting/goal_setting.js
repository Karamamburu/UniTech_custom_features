...
id_tab = 0;
rps_list = {};
access_to_goalmap = false;

const tabsIconAndText = [
    {
        id: 0,
        icon: './download_file.html?file_id=7091976414233622001',
    ...
    {
        id: 2,
        icon: './download_file.html?file_id=7091976571740700202',
    ...
    {
        id: 3,
        icon: './download_file.html?file_id=7091976625523684140',
        addElem: () => { },
        attr: '#gap',
...
sendRequest({ mode: 'access', goalmap_id: $('#year_selector option:selected').attr('goalmap_id') }, (resp) => access_to_goalmap = resp.access);

if (access_to_goalmap) {
    ...
    $('#ipr_comments_toggle').on('change', function () {
        $('#col_info_comments_body').css('display', this.checked ? 'block' : 'none');
    });
...
function sendToApproveDialog() {
    if (permissionRules.edit_result_main && !current_final_mark_id) {
    ...
    if ($('[baseGoal]').length != $('[checkedBaseGoal]').length && permissionRules.send_to_approve && !permissionRules.drop) {
        jAlert("Не заполнены обязательные поля", "Пожалуйста, проставь оценку по всем целям", true)
        return;
    }

    // fields =  $('#current_salary_label, #want_salary_label, #comment_by_grow_label')
    fields = $('#comment_by_grow_label, #skill_level_self_label, #potencial_coll_self_lable');

    for (i = 0; i < fields.length; i++) {
        if (!fields[i].innerHTML) {
            jAlert("Не заполнены обязательные поля", "Пожалуйста, заполни все обязательные поля на вкладке «Компетенции и потенциал»", true)
            return;
        }
    }

    need_count_goals = true;
    messageErrCountGoals = "";
    sendRequest({
        mode: 'goals',
        goalmap_id: $('#year_selector option:selected').attr('goalmap_id'),
        include_non_actual: false
        ...
        sendRequest({ mode: 'col_info', collaborator_id: current_collaborator_id }, (info) => {
            if (info.position == "Shift Supervisor" && goals.length >= 2) {
                return;
            }

            if (info.position == "Shift Supervisor") {
                messageErrCountGoals = "Пожалуйста, добавьте не менее двух актуальных целей";
                need_count_goals = false;
            } else if (goals.length < 3) {
                messageErrCountGoals = "Пожалуйста, добавьте не менее трех актуальных целей";
                need_count_goals = false;
            } 
        })
    });
    ...
    if (!need_count_goals) {
        jAlert("Не заполнены обязательные поля", messageErrCountGoals, true);
        return;
    }
...
function approve(data, comment) {
    if (permissionRules.approve_by_halfyear && $('#year_selector option:selected').attr('is_owner') != 'true') {
        if ($('#final_result_manager_comment').html() == '') {
            jAlert('Подтверждение оценки результатов', 'Пожалуйста, заполните комментарий для общей оценки выполнения целей.');
            return;
        }
    }
    ...
    sendRequest({
        ...
        if (permissionRules.approve_by_halfyear) {
            message = "Ты подтвердил промежуточную оценку результатов сотрудника " + fio + " и отправил форму ему на ознакомление. Обязательно договорись с сотрудником о встрече, на которой вы вместе обсудите его годовые цели, результаты, план развития и потенциал.";
            if ($('#year_selector option:selected').attr('is_ready_approved_y2') == 'true') {
                message = "Ты подтвердил годовую оценку результатов сотрудника " + fio + " и отправил форму ему на ознакомление. Обязательно договорись с сотрудником о встрече, на которой вы вместе обсудите его годовые цели, результаты, план развития и потенциал.";
            }
        }
...
function updateColInfo(info) {
    ...
    if (info.position_ru == '') {
        ...
        $('#col_info_position').html(info.position_ru);
    }
        ...
        $('#final_result_desc .desc')[0].innerHTML = `
            <div class="goal-result-value-body">
                ...
                <div class="goal-desc-value-row" style='display: ${!info.comment_manager ? 'none' : ''}'>
                    ...
                    <div id="final_result_manager_comment" class="goal-desc-value">${info.comment_manager}</div>
                </div>
            </div>
        `;
    }
...
function updateGoalmapComments(comments) {
    if (comments.length == 0) {
        $('#col_info_comments_body, #col_comments_toggle').css('display', 'none');
        return;
    }

    $('#col_info_comments_body, #col_comments_toggle').css('display', '');
    $('#ipr_comments_toggle').prop('checked', true);
        ...
        if (comment.is_my_comment) {
            comments_body.append(`
                <div class="ipr_sended_comment_body">
                    ...
                    <div class="ipr_comment_message"><pre>${comment.comment}</pre></div>
                </div>
            `);
        } else {
            comments_body.append(`
        ...
        sendRequest({ mode: 'permission_rules', goalmap_id: $('#year_selector option:selected').attr('goalmap_id') },
            (rules) => {
                if (rules.approve && !rules.edit_result_main) {
                    goal_elem.find("#button_edit_desc").text("Редактировать");
                }
            });
    ...
    goalmaps.forEach(goalmap => {
        year_selector_elem.append(`
                <option
                    ...
                    ${new Date().getFullYear() == goalmap.year ? 'selected ' : ''}>
						${goalmap.year}
				</option>
            `);
    });
...
function editUnlock(rules) {
    ...
    $('[id=button_edit_potenc_self], [id=button_edit_skill_self]').css('display', (rules.edit_psr ? '' : 'none'));
    ...
    $('#edit_mangaer').css('display', (rules.edit_manager? '' : 'none'))
...
function accessDenied() {
    var viewport = $('.viewport-col-info');
    viewport.empty();
    viewport.append(`
        <div class="col-info-header-body">
            <div class="col-info-header">Отсутствует доступ к карте целей</div>
            <div id="col_info_fullname" class="col-info">У вас отсутствует доступ к карте целей пользователя, так как карта целей не принадлежит вам или вы не являетесь руководителем владельца карты целей.</div>
        </div>
    `);
    $('.goals-body').remove();
}
...
function editManagerDialog() {
    sendRequest({ mode: 'col_info', collaborator_id: current_collaborator_id }, (info) => {
        if (info.position == 'RSC' || info.position == 'RSC Contractor') {
            show('Выберите руководителя', 'collaborator', !isNull(permissionRules.available_bosses) ? permissionRules.available_bosses.map(x => x.id).join(';'):0, false, changeAvailableBosses);
        }
    })
}
...
function editGoalResultDialog(goal) {
        ...
        buttons: [{
            ...
            click: () => {
                ...
                if ((!commentQ2 && !!resultQ2 && goalmapObj.attr('is_owner') == 'true')) {
                    jAlert("Не заполнены обязательные поля", "Заполните поле комментарий в итоговой годовой оценке")
                    return;
                }
    ...
    enableDialogTab(($('#year_selector option:selected').attr('is_ready_approved_y2') != 'true') ? (1) : (2));
        ...
        buttons: [{
            ...
            click: () => {
                ...
                if (goal_desc && goal_plan_date && desc_what_done && desc_how_done) {
                    ...
                    $('#ui_dialog').find('#goal_desc, #goal_plan_date, #goal_desc_what_done, #goal_desc_how_done').each(function() {
                        $(this).toggleClass('empty_field', $(this).val() === '');
                    });
    ...
    $(`
        <div id='ui_dialog' goal_id='${goal.id}' class="wt-lp-dialog-body">
            <div class="wt-lp-dialog-fld-container">
                ...
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_desc' class="autoresize" oninput="autoResize(this)" placeholder="Обязательное для заполнения поле">${goal.description}</textarea>
                </div>
            </div>
            <div class="wt-lp-dialog-fld-container">
                <label>Срок выполнения</label>
			...
			<div class="wt-lp-dialog-fld-container">
                <label>Что должно быть сделано</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_desc_what_done' class="autoresize" oninput="autoResize(this)" placeholder="Обязательное для заполнения поле">${goal.desc_what_done ? goal.desc_what_done : ''}</textarea>
                </div>
            </div>
			<div class="wt-lp-dialog-fld-container">
                <label>Как должно быть сделано</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_desc_how_done' class="autoresize" oninput="autoResize(this)" placeholder="Обязательное для заполнения поле">${goal.desc_how_done ? goal.desc_how_done : ''}</textarea>
                </div>
            </div>
            <div class="wt-lp-dialog-fld-container">
                <label>Комментарий сотрудника</label>
    ...
    $(`
        <div id='ui_dialog' goal_id='${goal.id}' class="wt-lp-dialog-body">
            ...
            <div class="wt-lp-dialog-fld-container">
                <label>Комментарий сотрудника</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_desc_comment_coll' readonly class="autoresize" oninput="autoResize(this)" placeholder="Обязательное для заполнения поле">${goal.desc_comment_coll ? goal.desc_comment_coll : ''}</textarea>
                </div>
            </div>
			<div class="wt-lp-dialog-fld-container">
                <label>Комментарий руководителя</label>
...
function getSkillData() {
    ...
    $('#ui_dialog .wt-lp-title-skill').each(function() {
        $(this).parent().toggleClass('empty_field', skillData.skill_type == undefined);
    })
    $('#ui_dialog #skill-descr').find('.dx-toolbar-before, #goal_desc').toggleClass('empty_field', isNull(skillData.description) && isNull(skillData.competence_id))
        ...
        buttons: [{
            ...
            click: () => {
                ...
                if (isNull(skillData.skill_type) || (isNull(skillData.description) && isNull(skillData.competence_id))) {
                    jAlert('Ошибка при добавлении', 'Невозможно добавить план развития. Пожалуйста, заполни обязательное поле  «Что буду развивать»!');
                    return;
                }
                ...
                if (goal_plan_date && goal_action && goal_resource) {
                    ...
                    $('#ui_dialog').find('#goal_plan_date, #goal_action, #goal_resource').each(function() {
                        $(this).toggleClass('empty_field', $(this).val() === '');
                    });
                    jAlert("Не заполнены обязательные поля", "Пожалуйста, заполни все обязательные поля")
                }
            ...
            $(this).change(function () {
                ...
                if ($(this).prop("checked")) {
                    ...
                    if (skillType == "soft") {
                        ...
                        $('#skill-descr').append(`<textarea id='goal_desc' placeholder="Обязательное для заполнения поле." class="autoresize" oninput="autoResize(this)"></textarea>`)
                    }
                }
            });
        });
    ...
    $(`
        <div id='ui_dialog' class="wt-lp-dialog-body">
			...
			<div class="wt-lp-dialog-fld-container">
                <label>Мой план действий</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_action' placeholder="Обязательное для заполнения поле." readonly class="autoresize" oninput="autoResize(this)">${ipr.actions ? ipr.actions : ''}</textarea>
                </div>
            </div>
            <div class="wt-lp-dialog-fld-container">
                <label>Ресурсы</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_resource' placeholder="Обязательное для заполнения поле." readonly class="autoresize" oninput="autoResize(this)">${ipr.resource ? ipr.resource : ''}</textarea>
                </div>
            </div>
            <div class="wt-lp-dialog-fld-container" style="display: none !important">
                <label>Обучающие программы</label>
            ...
            $(this).change(function () {
                ...
                if ($(this).prop("checked")) {
                    ...
                    if (skillType == "soft") {
                        ...
                        $('#skill-descr').append(`<textarea id='goal_desc' placeholder="Обязательное для заполнения поле." class="autoresize" readonly oninput="autoResize(this)">${ipr.description}</textarea>`)
                    }
                }
            });
        });
    ...
    `).dialog({
        ...
        buttons: [{
            ...
            click: () => {
                ...
                if ((!commentQ2 && !!resultQ2 && goalmapObj.attr('is_owner') == 'true')) {
                    jAlert("Не заполнены обязательные поля", "Пожалуйста, заполни поле комментарий в итоговой годовой оценке")
                    return;
                }
...
function editIPRDialog(ipr) {
    ...
    $(`
        <div id='ui_dialog' data-id="${ipr.id}" data-name="${ipr.name}" data-goalmap-id ="${ipr.goalmap_id ? ipr.goalmap_id : $('#year_selector option:selected').attr('goalmap_id')}" class="wt-lp-dialog-body">
			...
			<div class="wt-lp-dialog-fld-container">
                <label>Мой план действий</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_action' placeholder="Обязательное для заполнения поле." class="autoresize" oninput="autoResize(this)">${ipr.actions ? ipr.actions : ''}</textarea>
                </div>
            </div>
            <div class="wt-lp-dialog-fld-container">
                <label>Ресурсы</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_resource' placeholder="Обязательное для заполнения поле." class="autoresize" oninput="autoResize(this)">${ipr.resource ? ipr.resource : ''}</textarea>
                </div>
            </div>
            <div class="wt-lp-dialog-fld-container" style="display: none !important">
                <label>Обучающие программы</label>
            ...
            <div class="wt-lp-dialog-fld-container">
                <label>Комментарий сотрудника</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_desc_comment_coll' class="autoresize" oninput="autoResize(this)">${ipr.desc_comment_coll ? ipr.desc_comment_coll : ''}</textarea>
                </div>
            </div>
			<div class="wt-lp-dialog-fld-container">
                <label>Комментарий руководителя</label>
            ...
            $(this).change(function () {
                ...
                if ($(this).prop("checked")) {
                    ...
                    if (skillType == "soft") {
                        ...
                        $('#skill-descr').append(`<textarea id='goal_desc' placeholder="Обязательное для заполнения поле." class="autoresize" oninput="autoResize(this)">${ipr.description}</textarea>`)
                    }
                }
                if (is_owner != 'true') {
    ...
    if (is_owner == 'true') {
        ...
        $('#ui_dialog').find('#goal_desc_comment_coll').attr('readonly', false);
    }
}
...
function editPotencialDialog(rps) {
    ...
    $(`
        <div id='ui_dialog' data-goalmap-id ="${rps.goalmap_id ? rps.goalmap_id : $('#year_selector option:selected').attr('goalmap_id')}" class="wt-lp-dialog-body">
            <div class="wt-lp-dialog-fld-container">
                <label>Потенциал сотрудника ${is_owner == 'true' ? '- самооценка' : ''}</label>
    ...
    `).dialog({
        ...
        buttons: [{
            ...
            click: () => {
                ...
                saveGAP(is_owner == 'false' ? {
                    id: rps.goalmap_id,
                    potencial_coll_id: id
                } : {
                    id: rps.goalmap_id,
                    potencial_coll_self_id: id
                });
                $('#ui_dialog').remove();
                $('#dialog_background_overlay').css('display', 'none');
            }
    ...
    $('#rps-dialog-lookup').dxSelectBox({
        ...
        displayExpr: 'name',
        valueExpr: 'code',
        value: is_owner != 'true' ? rps.potencial_coll_code : rps.potencial_coll_self_code,
        onSelectionChanged(e) {
...
function editSkillDialog(rps) {
    ...
    $(`
        <div id='ui_dialog' data-goalmap-id ="${rps.goalmap_id ? rps.goalmap_id : $('#year_selector option:selected').attr('goalmap_id')}" class="wt-lp-dialog-body">
            <div class="wt-lp-dialog-fld-container">
                <label>Уровень профессионализма ${is_owner == 'true' ? '- самооценка' : ''}</label>
                <div class="wt-lp-dialog-fld" id="rps-dialog-lookup"></div>
            </div>
        </div>
    `).dialog({
    ...
    $('#rps-dialog-lookup').dxSelectBox({
        ...
        displayExpr: 'name',
        valueExpr: 'code',
        value: is_owner != 'true' ? rps.skill_level_code : rps.skill_level_self_code,
        onSelectionChanged(e) {
        ...
        buttons: [{
            ...
            click: () => {
                ...
                if (okr_desc && okr_plan_date && okr_actions) {
                    ...
                    $('#ui_dialog').find('#goal_desc, #goal_plan_date, #goal_action').each(function() {
                        $(this).toggleClass('empty_field', $(this).val() === '');
                    });
                    jAlert("Не заполнены обязательные поля", "Пожалуйста, заполни все обязательные поля")
                }
    ...
    $(`
        <div id='ui_dialog' class="wt-lp-dialog-body">
			...
			<div class="wt-lp-dialog-fld-container">
                <label>Мой вклад в OKR</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_action' placeholder="Обязательное для заполнения поле." readonly class="autoresize" oninput="autoResize(this)">${okr.actions ? okr.actions : ''}</textarea>
                </div>
            </div>
            <div class="wt-lp-dialog-fld-container">
                <label>Комментарий сотрудника</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_desc_comment_coll' readonly class="autoresize" oninput="autoResize(this)" placeholder="Обязательное для заполнения поле">${okr.desc_comment_coll ? okr.desc_comment_coll : ''}</textarea>
                </div>
            </div>
			<div class="wt-lp-dialog-fld-container">
                <label>Комментарий руководителя</label>
        ...
        $('#dialog_tabs').append(`
            <div id='tab_y${number}' tab_number='${number}' class='dialog-tab'>
                ...
                <div class="wt-lp-dialog-fld-container">
                    <label>Комментарий сотрудника:</label>
                    <div class="wt-lp-dialog-fld">
                        <textarea id='y${number}_comment' class="autoresize" oninput="autoResize(this)" placeholder="Обязательное для заполнения поле">${okr[`y${number}_comment`]}</textarea>
                    </div>
                </div>
				<div class="wt-lp-dialog-fld-container">
					<label>Комментарий руководителя:</label>
    ...
    $(`
        <div id='ui_dialog' class="wt-lp-dialog-body">
			...
			<div class="wt-lp-dialog-fld-container">
                <label>Мой вклад в OKR</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='goal_action' placeholder="Обязательное для заполнения поле." class="autoresize" oninput="autoResize(this)">${okr.actions ? okr.actions : ''}</textarea>
                </div>
            </div>
            <div class="wt-lp-dialog-fld-container">
                <label>Комментарий сотрудника</label>
...
function updateOKRS(okrs) {
    ...
    okrs.forEach(okr => {
        ...
        sendRequest({ mode: 'permission_rules', goalmap_id: $('#year_selector option:selected').attr('goalmap_id') },
            (rules) => {
                if (rules.approve && !rules.edit_result_main) {
                    okr_elem.find("#button_edit_desc").text("Редактировать");
                }
            });
...
function updateIPRS(iprs) {
    ...
    iprs.forEach(ipr => {
        ...
        sendRequest({ mode: 'permission_rules', goalmap_id: $('#year_selector option:selected').attr('goalmap_id') },
            (rules) => {
                if (rules.approve && !rules.edit_result_main) {
                    ipr_elem.find("#button_edit_desc").text("Редактировать");
                }
            });
...
function updateGAP(goalmap) {
    ...
    gap_elem.append($(`
        <div class="goal goal-potential">
            <div class="goal-potential-potential-block">
                <div class="goal-header-body">
                    <div class="goal-header">
                        <div class="goal-header-text">Потенциал сотрудника - самооценка</div>
                    </div>
                    <div class="goal-buttons" style='display: ${is_owner == 'false' ? 'none !important' : ''}'>
                        <button id="button_edit_potenc_self" class="goal-edit-button">Редактировать</button>
                    </div>
                </div>
                <div class="goal-desc-body">
                    <div class="goal-desc" id="potencial_coll_self_lable" placeholder="Обязательное для заполнения поле">${goalmap.potencial_coll_self_name}</div>
                </div>
            </div>

            <div class="goal-potential-potential-block" style='display: ${is_owner == 'true' ? 'none !important' : ''}'>
                <div class="goal-header-body">
                    <div class="goal-header">
            ...
            <div class="goal-potential-prof-block">
                <div class="goal-header-body">
                    <div class="goal-header">
                        <div class="goal-header-text">Уровень профессионализма - самооценка</div>
                    </div>
                    <div class="goal-buttons" style='display: ${is_owner == 'false' ? 'none !important' : ''}'>
                        <button id="button_edit_skill_self" class="goal-edit-button">Редактировать</button>
                    </div>
                </div>
                <div class="goal-desc-body">
                    <div class="goal-desc" id="skill_level_self_label" placeholder="Обязательное для заполнения поле">${goalmap.skill_level_self_name}</div>
                </div>
            </div>

            <div class="goal-potential-prof-block" style='display: ${is_owner == 'true' ? 'none !important' : ''}'>
                <div class="goal-header-body">
                    <div class="goal-header">
            ...
            <div class="goal-potential-comment-coworker-block">
                ...
                <div class="goal-desc-body">
                    <div class="goal-desc" id="comment_by_grow_label" placeholder="Обязательное для заполнения поле"><pre>${goalmap.comment_by_grow}</pre></div>
                </div>
            </div>
            ...
            <div class="goal-potential-comment-namager-block">
                ...
                <div class="goal-desc-body" style='display: ${is_owner == 'true' ? 'none !important' : ''}'>
                    <div class="goal-desc" id="comment_manager_by_grow_label" placeholder="Обязательное для заполнения поле"><pre>${goalmap.comment_manager_by_grow}</pre></div>
                </div>
            </div>
        </div>
    `));
    ...
    gap_elem.find('#button_edit_potenc_self').click(function () {
        editPotencialDialog(goalmap);
    });

    gap_elem.find('#button_edit_skill_self').click(function () {
        editSkillDialog(goalmap);
    });
    ...
    sendRequest({
        ...
        $('#dialog_tabs').append(`
            ...
            <div class="wt-lp-dialog-fld-container">
                <label>Комментарий руководителя:</label>
                <div class="wt-lp-dialog-fld">
                    <textarea id='final_comment_manager' class="autoresize" oninput="autoResize(this)" ${is_owner != 'true' ? 'placeholder="Обязательное для заполнения поле"' : ''}>${info.comment_manager}</textarea>
                </div>
            </div>
        `);
...
function createReportHtmlCode() {
    ...
    <style>
        ...
        .goal-result-value-row {
            display: table;
        }

        .goal-result-sub-header {
            display: table-cell;
        }
